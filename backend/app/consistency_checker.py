...
class DataConsistencyChecker:
    def __init__(self, check_interval_seconds=10, quiet_period_seconds=15):
        ...
        self.quiet_period = quiet_period_seconds  # How long to wait after last write before checking
    ...
    async def start_monitoring(self):
        ...
        logger.info(f"Data consistency checker started (interval: {self.check_interval}s, quiet period: {self.quiet_period}s)")
    ...
    async def _monitor_loop(self):
        ...
        try:
            while self.is_running:
                ...
                write_activity_since_last_check = self.last_write_time > self.last_check_time
                
                # Check if enough time has passed since last write (quiet period)
                time_since_last_write = now - self.last_write_time
                quiet_period_elapsed = time_since_last_write >= timedelta(seconds=self.quiet_period)
                
                if write_activity_since_last_check and quiet_period_elapsed:
                    logger.info(f"Write activity detected since last check and quiet period elapsed ({time_since_last_write.total_seconds():.1f}s), running consistency check...")
                    await self.run_consistency_check()
                    self.last_check_time = now
                elif write_activity_since_last_check:
                    logger.debug(f"Write activity detected but still in quiet period (waiting {self.quiet_period - time_since_last_write.total_seconds():.1f}s more)")
    ...
    async def run_consistency_check(self):
        ...
        try:
            ...
            duplicates_fixed = await self._fix_duplicate_active_reservations(data)
            ...
            reservation_status_fixes = await self._sync_reservation_statuses(data)
            
            # Step 4: Final validation
            await self._validate_final_state(data)
            
            if duplicates_fixed > 0 or book_status_fixes > 0 or reservation_status_fixes > 0:
                logger.warning(
    ...
    async def _load_all_data(self):
        ...
        try:
            ...
            active_reservations = await execute_async(SimpleStatement("SELECT * FROM reservations_user_book", fetch_size=100))
            all_reservations = await execute_async("SELECT * FROM reservations")
            all_books = await execute_async("SELECT * FROM books")
            reservations_by_user = await execute_async("SELECT * FROM reservations_by_user")
            reservations_by_book = await execute_async("SELECT * FROM reservations_by_book")
            
            return {
                'active_reservations': list(active_reservations) if active_reservations else [],
                'all_reservations': list(all_reservations) if all_reservations else [],
                'all_books': list(all_books) if all_books else [],
                'reservations_by_user': list(reservations_by_user) if reservations_by_user else [],
                'reservations_by_book': list(reservations_by_book) if reservations_by_book else []
            }
        ...
        try:
            ...
            for book_id, reservations in book_reservations.items():
                if len(reservations) > 1:
                    logger.warning(f"Found {len(reservations)} active reservations for book {book_id}")
                    ...
                    logger.info(f"Keeping reservation {keep_reservation['reservation_id']}, cancelling {len(cancel_reservations)} others")
        ...
        try:
            ...
            for book in all_books:
                ...
                if should_be_checked_out and current_status != 'checked_out':
                    logger.warning(f"Book {book_id_str} ({book.title}) should be checked_out but is '{current_status}'")
                    update_query = "UPDATE books SET status = 'checked_out' WHERE book_id = %s"
                    await execute_async(update_query, (book.book_id,))
                    fixed_count += 1
                    
                elif not should_be_checked_out and current_status != 'available':
                    logger.warning(f"Book {book_id_str} ({book.title}) should be available but is '{current_status}'")
                    update_query = "UPDATE books SET status = 'available' WHERE book_id = %s"
                    await execute_async(update_query, (book.book_id,))
                    fixed_count += 1
    ...
    async def _sync_reservation_statuses(self, data):
        ...
        try:
            ...
            active_reservation_ids = {str(row.reservation_id) for row in active_reservations}
            ...
            for reservation in all_reservations:
                ...
                if should_be_active and current_status != 'active':
                    logger.warning(f"Reservation {reservation_id_str} should be active but is '{current_status}'")
                    update_query = """
                        UPDATE reservations 
                        SET status = 'active', updated_at = %s 
                        WHERE reservation_id = %s
                    """
                    await execute_async(update_query, (datetime.utcnow(), reservation.reservation_id))
                    fixed_count += 1
                    
                elif not should_be_active and current_status == 'active':
                    logger.warning(f"Reservation {reservation_id_str} should be completed but is active")
                    update_query = """
                        UPDATE reservations 
                        SET status = 'completed', updated_at = %s 
                        WHERE reservation_id = %s
            ...
            for reservation in reservations_by_user:
                ...
                if should_be_active and current_status != 'active':
                    update_query = """
                        ...
                        WHERE user_id = %s AND reservation_id = %s
                    """
                    await execute_async(update_query, (reservation.user_id, reservation.reservation_id))
                    fixed_count += 1
            ...
            for reservation in reservations_by_book:
                ...
                if should_be_active and current_status != 'active':
                    update_query = """
                        ...
                        WHERE book_id = %s AND reservation_id = %s
                    """
                    await execute_async(update_query, (reservation.book_id, reservation.reservation_id))
                    fixed_count += 1
    ...
    async def _validate_final_state(self, data):
        ...
        try:
            ...
            checked_out_books = [book for book in all_books if book.status == 'checked_out']
            checked_count = len(checked_out_books)
            
            logger.info(f"Final state validation:")
            logger.info(f"  Active reservations in user_book table: {user_book_count}")
            logger.info(f"  Checked out books: {checked_count}")
            
            # These counts should match
            if user_book_count != checked_count:
                logger.error(f"CONSISTENCY ERROR: Active reservations ({user_book_count}) don't match checked out books ({checked_count})")
            else:
                logger.info("✅ Active reservations match checked out books - core data is consistent")
            
            # Check for duplicate active reservations (books reserved by multiple users)
            book_reservation_count = {}
            for reservation in active_reservations:
                book_id = str(reservation.book_id)
                book_reservation_count[book_id] = book_reservation_count.get(book_id, 0) + 1
            
            duplicates_found = 0
            for book_id, count in book_reservation_count.items():
                if count > 1:
                    logger.error(f"DUPLICATE RESERVATION: Book {book_id} has {count} active reservations")
                    duplicates_found += 1
            
            if duplicates_found == 0:
                logger.info("✅ No duplicate active reservations found")
            else:
                logger.error(f"❌ Found {duplicates_found} books with duplicate active reservations")
